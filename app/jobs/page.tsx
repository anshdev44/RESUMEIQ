  ...
  const fetchJobs = async () => {
    try {
      ...
      const res = await fetch(
        `http://localhost:3000/api/Getjobs?role=frontend`,
      );
      const clean = await res.json();
      console.log("ye hai asli", clean);
    } catch (error) {
      ...
      <main className="max-w-5xl mx-auto px-6 py-12 md:py-20">
        <div className="mb-12 animate-fade-up">
          ...
          <p className="text-xl text-gray-500">
            ...
            based on a 92/100 profile score.
          </p>
        </div>
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  <span className="px-3 py-1.5 bg-gray-50 border border-gray-100 rounded-lg text-xs font-medium text-gray-700">
                    React
                  </span>
                  <span className="px-3 py-1.5 bg-gray-50 border border-gray-100 rounded-lg text-xs font-medium text-gray-700">
                    Node.js
              ...
              <button
                ...
                className="px-8 py-3 bg-black text-white font-medium rounded-full shadow-lg shadow-black/10 hover:bg-gray-800 transition-all shrink-0"
              >
                Apply Now
              </button>
            </div>
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
              <div>
                <div className="flex flex-wrap items-center gap-3 mb-3">
                  ...
                  <span className="px-3 py-1 bg-yellow-50 text-yellow-700 border border-yellow-200 text-xs font-bold rounded-full">
                    87% Match
                  </span>
                  <span className="px-3 py-1 bg-gray-50 text-gray-600 border border-gray-200 text-xs font-semibold rounded-full">
                    Hybrid
