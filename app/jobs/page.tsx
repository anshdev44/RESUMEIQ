...
const JobCard = ({ job, matchPercentage }: { job: any, matchPercentage?: number }) => {
  return (
    <div className="p-8 rounded-3xl border border-gray-200 dark:border-gray-800 hover:border-gray-300 dark:hover:border-gray-700 hover:shadow-xl hover:shadow-black/[0.04] transition-all bg-white dark:bg-gray-900 group relative overflow-hidden flex flex-col h-full">
      <div className="absolute top-0 left-0 w-1 h-full bg-black dark:bg-white group-hover:bg-indigo-500 transition-colors" />
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 flex-1">
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-3 mb-3">
            ...
            {matchPercentage && (
                <span className="px-3 py-1 bg-green-50 dark:bg-green-900/40 text-green-700 dark:text-green-400 border border-green-200 dark:border-green-800 text-xs font-bold rounded-full">
                  {matchPercentage}% Match
                </span>
            )}
            {job.job_is_remote && (
              <span className="px-3 py-1 bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 text-xs font-semibold rounded-full">
                Remote
              </span>
            )}
            {job.job_employment_type && (
              <span className="px-3 py-1 bg-purple-50 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800 text-xs font-semibold rounded-full">
                {job.job_employment_type}
              </span>
            )}
          </div>
        ...
        <Link
          ...
          target="_blank"
          className="cursor-pointer px-8 py-3 bg-black dark:bg-white text-white dark:text-black font-medium rounded-full shadow-lg hover:bg-gray-800 dark:hover:bg-gray-200 transition-all shrink-0 text-center"
        >
          Apply Now
        </Link>
      </div>
            ...
            {hasMore && (
                <div className="mt-8 text-center">
                    <button 
                        ...
                        className="cursor-pointer px-6 py-2 border border-gray-300 dark:border-gray-700 rounded-full font-medium text-gray-600 dark:text-gray-400 hover:text-black dark:hover:text-white hover:border-gray-500 dark:hover:border-gray-500 transition-all bg-white dark:bg-gray-900"
                    >
...
const JobsPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-purple-50 to-white dark:from-purple-950 dark:via-black dark:to-fuchsia-950 text-black dark:text-gray-100 font-sans pb-20 animate-fade-in">
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 group">
            <div className="w-8 h-8 bg-black dark:bg-white rounded-lg flex items-center justify-center group-hover:scale-105 transition-transform">
              <span className="text-white dark:text-black font-bold text-xs">IQ</span>
            </div>
            ...
            {session ? (
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="cursor-pointer text-sm font-medium px-4 py-2 rounded-full border border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-400 hover:text-black dark:hover:text-white hover:border-gray-400 dark:hover:border-gray-600 transition-all bg-white dark:bg-black"
                >
                  Sign Out
                </button>
              </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-12 md:py-20">
        <div className="mb-12 animate-fade-up">
          <Link
            href="/analysis"
            className="inline-flex items-center text-sm font-medium text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white mb-8 transition-colors group"
          >
            <div className="w-8 h-8 rounded-full border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 flex items-center justify-center mr-3 group-hover:border-black dark:group-hover:border-white transition-colors">
              <svg
                className="w-4 h-4"
                fill="none"
                viewBox="0 0 24 24"
        ...
        {hasFetched && !isLoading && res_job_1.length === 0 && res_job_2.length === 0 && res_job_3.length === 0 && (
            <div className="text-center py-20 bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-sm animate-fade-up">
        ...
        {!hasFetched && !isLoading && (
            <div className="text-center py-24 bg-white dark:bg-gray-900 rounded-3xl border border-gray-200 dark:border-gray-800 shadow-sm animate-fade-up delay-100 flex flex-col items-center justify-center">
                ...
                <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Ready to find your next role?</h3>
                ...
                <button
                    onClick={fetchJobsNames}
                    className="cursor-pointer px-8 py-3 bg-black dark:bg-white text-white dark:text-black font-medium rounded-full hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors shadow-lg"
                >
                    Start Job Search
                </button>
            </div>
